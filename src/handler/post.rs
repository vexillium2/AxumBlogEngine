...
fn convert_post_model_to_post_info(model: PostModel) -> PostInfo {
    PostInfo {
        id: model.id,
        title: model.title,
...
pub async fn get_posts_list_and_search(
    AuthUser(claims): Option<AuthUser>, // 允许未认证用户访问，但如果认证了则可以检查权限
    State(state): State<AppState>,
    Query(mut req): Query<PostListRequest>, // 使用 Query 提取查询参数
) -> CustomResult<Json<PostListResponse>> {
    // 如果是普通用户访问，确保只获取已发布的文章
    if claims.is_none() || (claims.is_some() && claims.as_ref().unwrap().role != "admin") {
        req.published_only = Some(true); // 强制设置为 true
    }
    // 如果是管理员访问，published_only 按照请求参数来决定（默认为 true）

    let (posts_models, total_pages, current_page, total_posts) = 
        state.post_repo.get_paginated_posts(req).await?;

    let posts_info: Vec<PostInfo> = posts_models
        .into_iter()
        .map(convert_post_model_to_post_info)
        .collect();
