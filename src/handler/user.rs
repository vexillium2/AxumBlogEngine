...
/// JWT Claims 结构体，用于从令牌中提取用户 ID 和角色。
/// `sub` (subject) 通常用于存储用户唯一标识（这里是用户ID的字符串形式）。
#[derive(Debug, serde::Serialize, serde::Deserialize)]
pub struct Claims {
    pub sub: String, // 用户ID的字符串形式
    pub username: String, // 可以在token中包含用户名，方便快速访问，但注意数据膨胀
    pub role: String,
...
pub async fn register_user(
    ...
    let created_user = state.user_repo.create_user_from_register(req, hashed_password).await
        .map_err(|e| CustomError::DbError(e.to_string()))?; // 转换为 DbError
