...
// 导入数据库模块的 PostRepository，用于检查文章是否存在
use crate::database::post::PostRepository;

// 导入 IDL 中定义的请求和响应 DTOs
use crate::handler::idl::{
    ToggleFavoriteRequest, FavoriteListResponse, BaseResponse, PostInfo,
};
// 导入自定义错误类型和通用 Result
...
// 导入文章模型转换函数
use crate::handler::post::convert_post_model_to_post_info; // 假设此函数在 post 模块中已公开
...
pub async fn get_my_favorite_posts(
    ...
    let (favorited_posts_models, total_pages, current_page, total_favorites) =
    ...
    let posts_info: Vec<PostInfo> = favorited_posts_models
        .into_iter()
        .map(convert_post_model_to_post_info) // 使用已公开的转换函数
        .collect();
